...
`)}\n</ol>`;case`image`:{let t=e.alt?` alt="${E(e.alt)}"`:``,n=e.width?` width="${e.width}"`:``,r=e.height?` height="${e.height}"`:``;return`<img src="${E(e.src)}"${t}${n}${r}>`}case`hr`:return`<hr>`;case`callout`:{let t=T(e.children);return`<div class="oe-callout oe-callout-${e.variant}" data-callout-variant="${e.variant}">${t||`<br>`}</div>`}default:return``}}function T(e){return e.map(ce).join(``)}function ce(e){if(e.type===`hardbreak`)return`<br>`;let t=de(e.text);if(t===``&&e.marks.length===0)return``;let n=[...e.marks].sort(le);for(let e of n)t=ue(t,e);return t}function le(e,t){let n=[`strikethrough`,`underline`,`italic`,`bold`,`link`,`code`];return n.indexOf(e.type)-n.indexOf(t.type)}function ue(e,t){switch(t.type){case`bold`:return`<strong>${e}</strong>`;case`italic`:return`<em>${e}</em>`;case`underline`:return`<u>${e}</u>`;case`code`:return`<code>${e}</code>`;case`strikethrough`:return`<s>${e}</s>`;case`link`:{let n=t,r=n.target===`_blank`?` target="_blank" rel="noopener noreferrer"`:``,i=n.title?` title="${E(n.title)}"`:``;return`<a href="${E(n.href)}"${r}${i}>${e}</a>`}default:return e}}function de(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function E(e){return e.replace(/&/g,`&amp;`).replace(/"/g,`&quot;`).replace(/</g,`&lt;`)}function D(e){return e.children.map(fe).filter(Boolean).join(`

`)}function fe(e){switch(e.type){case`paragraph`:return O(e.children);case`heading`:return`${`#`.repeat(e.level)} ${O(e.children)}`;case`blockquote`:return e.children.map(fe).join(`

`).split(`
`).map(e=>`> ${e}`).join(`
`);case`code_block`:return`\`\`\`${e.lang??``}\n${e.children.map(e=>e.text).join(``)}\n\`\`\``;case`bullet_list`:return e.children.map(e=>`- ${O(e.children)}`).join(`
`);case`ordered_list`:{let t=e.start??1;return e.children.map((e,n)=>`${t+n}. ${O(e.children)}`).join(`
`)}case`image`:return`![${e.alt??``}](${e.src})`;case`hr`:return`---`;case`callout`:return`> [!${e.variant??`info`}]\n${O(e.children).split(`
`).map(e=>e?`> ${e}`:`>`).join(`
`)}`;default:return``}}function O(e){return e.map(pe).join(``)}function pe(e){if(e.type===`hardbreak`)return`
`;let t=e.text,n=[...e.marks].sort((e,t)=>{let n=[`link`,`bold`,`italic`,`strikethrough`,`underline`,`code`];return n.indexOf(e.type)-n.indexOf(t.type)});for(let e of n)switch(e.type){case`bold`:t=`**${t}**`;break;case`italic`:t=`*${t}*`;break;case`code`:t=`\`${t}\``;break;case`strikethrough`:t=`~~${t}~~`;break;case`underline`:t=`<u>${t}</u>`;break;case`link`:{let n=e,r=n.title?` "${n.title}"`:``;t=`[${t}](${n.href}${r})`;break}}return t}function k(e){if(!e||e.trim()===``)return p();let t=me(e);return t.length>0?{children:t}:p()}function me(e){let t=e.split(`
